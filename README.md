...
- [v0.1.24](https://github.com/Yummn/sts2-turn-rewind/releases/tag/v0.1.24)：快照新增怪物精确坐标/缩放、卡牌子类运行时字段、动态数值与牌组原型状态，并在恢复后将战斗历史重新绑定到新卡牌实例。修复回溯后怪物模型上移，以及储君铸剑、爪击、暴走等本场战斗累计伤害或按自身出牌次数计算的效果归零。提供 Android v111（启动器 1.9）兼容包；PC v0.107.1 已完成真实出牌、累计铸剑、历史引用和怪物坐标联合回归测试。
