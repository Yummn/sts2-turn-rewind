...
- [v0.1.28](https://github.com/Yummn/sts2-turn-rewind/releases/tag/v0.1.28)：快照并恢复能力内部计数与动态数值，修复储君环绕轨道的回能进度、虚空形态的本回合免费次数在回溯后重置。PC v0.107.1 实战测试同时验证王者之踢的抽牌减费、自动化、野性和华丽收场的状态恢复；Android v0.110.1 已编译，未实机测试。
...
- [v0.1.24](https://github.com/Yummn/sts2-turn-rewind/releases/tag/v0.1.24)：快照新增怪物精确坐标/缩放、卡牌子类运行时字段、动态数值与牌组原型状态，并在恢复后将战斗历史重新绑定到新卡牌实例。修复回溯后怪物模型上移，以及储君铸剑、爪击、暴走等本场战斗累计伤害或按自身出牌次数计算的效果归零。PC v0.107.1 已完成真实出牌、累计铸剑、历史引用和怪物坐标联合回归测试。v0.1.24 的 v111 文件是使用 v110.1 引用构建的兼容候选，并非独立使用 v111 程序集编译或实机验证的原生版本。
